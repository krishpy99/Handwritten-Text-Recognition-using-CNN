...
def _crop_add_border(img, height, threshold=50, border=True, border_size=15):
    ...
    if height != 0:
        ...
        img = img[y0:y1, x0:x1]
    if border:
        return cv2.copyMakeBorder(img, 0, 0, border_size, border_size, cv2.BORDER_CONSTANT, value=[0, 0, 0])
    return img
...
def letter_normalization(image, is_thresh=True, dim=False):
    ...
    result[offset[1]:offset[1] + resized.shape[0], offset[0]:offset[0] + resized.shape[1]] = resized
    if dim:
